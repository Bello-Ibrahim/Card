...
## Hook
A food delivery app tells you two things: "Your order is likely to be on time" and "Arrives in about 35 minutes." Both answers can come from machine learning, but one is a category and the other is a number. This lesson explains why that difference matters.

## Explanation
In L03 and L04 you learned that a model trains on examples with features and labels, and then makes predictions about new cases. When every training example comes with the correct answer attached, we call this **supervised learning**. The labels act as an answer key.

Supervised learning has two main types, depending on what kind of answer you want.
...
The type matters because it changes the labels you collect and how you judge the model. For classification, you check how often the model picks the right box. For regression, you check how close its number is to the real one.

## Worked Example
Andrés manages a small courier company in Medellín, Colombia. Customers often call to ask when their parcels will arrive. He has records of 5,000 past deliveries, with features such as distance, time of day, weather and neighbourhood.
