...
**2. Label (annotate) them.** Use a free annotation tool, such as Label Studio or CVAT, to draw a tight box around every visible object of each class. [VERSION] Label *every* object, including partly hidden ones; an unlabelled object teaches the model that it is background.
...
The numbers are the class ID, box centre x, centre y, width and height, scaled from 0 to 1 by the image size. Export options change, so check a few exported files. [VERSION] A small YAML file tells YOLO where the images are and what the classes are called:
...
## Worked Example
Mei Ling Chen runs convenience shops in Kuala Lumpur, Malaysia. She wants to detect two products: juice cartons and rice bags. She takes 60 shelf photos, with no customers in view, and labels them in Label Studio.
...
4. In Colab, unzip it into `/content/shelf` (with `images/` and `labels/` split into `train` and `val`) and create `shelf.yaml`.
...
Three failures stand out: rice bags on the bottom shelf, seen from above, are missed; two cartons side by side get one large box; and a carton behind a price label is not found. She will add 20 bottom-shelf photos and check that neighbouring cartons have separate boxes in her labels.
...
**What good looks like:** A YOLO-format dataset, a completed training run, both mAP values, and 3 failure cases, each linked to a cause such as angle, overlap, lighting or labelling.
...
- [VERIFY] Curriculum flag: confirm the Ultralytics licence (AGPL-3.0 with a separate commercial licence) before scripting.
