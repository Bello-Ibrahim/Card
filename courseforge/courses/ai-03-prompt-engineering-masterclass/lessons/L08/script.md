# L08 Thinking Tasks: Analysis, Planning and Decisions | Presenter Script

Course: AI-03 · Video: 5 min · Words: 640

## Hook
Which option should I choose? AI tools are happy to answer that question, often with great confidence. But a confident answer is only as good as the reasoning and the information behind it.

## Explain
So far, you have used AI for writing, where you mostly check the wording. For thinking tasks, such as analysis, planning and decisions, you need to see the reasoning. Four prompt techniques often help.

Technique one: ask it to work step by step before it gives a recommendation. This makes the reasoning visible, so you can check each step. With many tools this often helps, but it does not guarantee a correct answer. Technique two: ask it to list the assumptions it is making. Seeing them lets you correct the wrong ones.

Technique three: name the criteria. Instead of asking which is best, ask the tool to compare the options on cost, reliability and ease of use, in a table. Your criteria reflect your priorities, which the tool cannot know. Technique four: ask it to ask you up to five questions before it answers.

After the analysis, one more request is very useful. Ask: what is the weakest assumption in your analysis? This asks the tool to question its own answer, and it often shows where you need to check facts yourself.

Think of a well-read friend helping you compare three flats to rent. They can organise your thinking, and point out things you forgot. But they have not visited the flats. So you still check the facts before you sign. The tool does not know current prices, local conditions or your finances unless you tell it.

## Demonstrate
Let's try it. Ravi runs a small café in Chennai. He wants to start delivery, and he is comparing three delivery partners. He has their offers written down.

His first prompt asks: which delivery partner is best for my café? The answer is a general list of things to consider, with no clear recommendation.

His second prompt describes the café: delivery within five kilometres, about thirty orders a day, mostly at lunch. It names three criteria: cost per order, lunchtime delivery time, and control over customer service. It includes each partner's terms. Then it asks the tool to work step by step, list its assumptions, show a table, and give a two-sentence recommendation.

The tool lists its assumptions. One is an average order value, and Ravi corrects it with his real figure. Then he asks for the weakest assumption. The tool says it had no information about lunchtime delivery times, and simply assumed they were equal for all three.

So Ravi decides to test all three partners for one week before he chooses. The AI did not make the decision. It organised the information, and showed Ravi what he still needed to find out.

A common mistake is to ask what should I do, and follow the recommendation without reading the reasoning. A recommendation can sound certain, even when it rests on a guessed number. Read the assumptions, correct the wrong ones, and ask for the weakest one before you act.

## Recap
Let's recap. First, for thinking tasks, ask the tool to work step by step, list its assumptions, and compare options on criteria you name. Second, asking for the weakest assumption often shows what you need to check. Third, step-by-step prompts often help, but they do not guarantee a correct answer. The decision and the fact-checking stay with you.

## CTA
Now it is your turn. In the exercise below this video, you will use a step-by-step prompt to compare two or three options for a real decision at work. Then you will ask the AI for the weakest assumption in its own analysis. It takes about twenty-five minutes. In the next lesson, we look at roles and multi-turn conversations. See you there.

## Thumbnail
Headline: Show Me Your Reasoning
Image: Navy background, a comparison table of three options with an assumptions list beside it and one assumption highlighted in amber, headline in teal Inter Bold.

## Production Notes
- [VERIFY] The claim that asking for step-by-step reasoning improves results varies between models. The voiceover says only that it 'often helps' and 'does not guarantee a correct answer'; do not change this wording to a stronger claim.
- [VERSION] Free-tier availability of Claude, ChatGPT and Gemini must be checked before recording (content.md Review Flags).
- Ravi, his café in Chennai and delivery partners A, B and C are hypothetical. Do not show real delivery-company names, uniforms or logos in stock footage.
- Before/after prompts appear on code slides with the exact text from content.md, including [FEES AND TERMS] and [AMOUNT]; the voiceover summarises them.
