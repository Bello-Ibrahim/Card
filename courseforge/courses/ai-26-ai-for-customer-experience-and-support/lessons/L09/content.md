...
## Hook
You have a clean knowledge base, a tone guide and a set of hand-off rules. This week you put them together into a working support assistant. The secret of a good first version is not to make it clever. It is to make it small and honest.

## Explanation
You will use **a free chatbot builder of your choice** [VERSION]. Many builders let you create an AI assistant in a web browser, add your own content and test it in a preview window, with no code. Interfaces differ between builders, so this lesson describes the four decisions that every builder asks you to make.

**1. Scope.** Decide which topics the assistant covers. Choose 3 to 5 frequent, low-risk topics from your L01 list, for example opening hours, order tracking and returns. Everything else is out of scope and goes to a person. A small assistant that answers three topics well is more useful than a large one that answers twenty topics badly.

**2. Knowledge.** Add the articles you wrote in L03, or similar clean articles for your made-up company. Most builders let you paste text, upload a document or add a web page [VERSION]. Add only the content for your scope. Use made-up company content only: do not upload real customer data or confidential documents to a free tool.
...
**4. Transparency.** Tell customers they are talking to AI. Put it in the welcome message, for example: "Hi, I am Kestrel's AI assistant. I can help with opening hours and repairs. You can ask for a person at any time." Being open builds trust, and in some regions the law requires it, for example under the EU AI Act transparency rules [REGION] [VERIFY]. You will look at this again in L12.

**Analogy:** Opening a support assistant is like opening a small food stall before a full restaurant. You serve a few dishes you can make well, you tell people clearly what is on the menu, and you point them to the restaurant next door for anything else. Once the stall runs well, you add dishes one at a time.

## Worked Example
Leila works at a hypothetical language school in Amman, Jordan. Students often ask about course dates, levels and how to pay. She builds an assistant that covers three topics: course start dates, how to choose a level, and payment methods.
...
During testing, Noor answers the visa question with general advice about embassies. Leila adds a line to the instructions: "Do not give advice on visas or legal matters; offer a person." She tests again and the problem is fixed.

## Common Mistake
Many beginners add every document they have to the assistant, hoping it will answer everything. More content, especially old or overlapping content, gives the assistant more chances to find the wrong text. Keep the first version small, test it well, and add new topics only when the current ones work.
...
**What good looks like:** A working assistant in test mode that answers at least 3 topics correctly from your articles, clearly says it is AI, and offers a person for out-of-scope questions. No real customer data is used.
**Time:** about 45 minutes

## Review Flags
- [VERSION] The brief names no chatbot builder: a reviewer must choose one or two builders with a usable free tier and then check sign-up, the names of the knowledge, instructions and preview sections, and upload options before recording.
...
- Reviewer check: the Arabic sample question should be checked by a native speaker.
